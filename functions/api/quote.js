 ...
 * Cloudflare Pages Function  |  Route: /api/quote
 *
 * GET  /api/quote?make=Toyota&model=RAV4&oil_type=full_synthetic
 * POST /api/quote  { "make": "Toyota", "model": "RAV4", "oil_type": "full_synthetic" }
 *
 * oil_type values: "conventional" | "synthetic_blend" | "full_synthetic"
 *   If omitted, defaults to the vehicle's manufacturer-recommended type.
 */

// ─── PRICING CONFIG ──────────────────────────────────────────────────────────
const PRICING = {
  base: 99,          // covers up to 5 qts of synthetic-blend, filter & labor
  extra_quart: 8,    // per quart over 5
  oil_upcharge: {
    conventional:    -2,  // slight discount vs synthetic-blend
    synthetic_blend:  0,  // base price (included in $99)
...
function calcPrice(vehicle, requestedOilType) {
  ...
  return { quarts, oil_type_used: oilType, base_price: PRICING.base,
           extra_quart_charge: extraCharge + oilUpcharge, total_estimate: total,
           price_range: `$${total}` };
}
...
export async function onRequest(context) {
  ...
  if (!vehicle) {
    return json({
      found: false, make, model, year: year || null,
  ...
  if (vehicle.electric) {
    return json({
      found: true, electric: true, make, model, year: year || null,
  ...
  if (oil_type && !validTypes.includes(oil_type)) oil_type = "";
